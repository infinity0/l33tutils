#!/bin/sh
# Simple way to update a Gandi domain via LiveDNS.
#
# Requires stun(1) from the stun-client package.
#
# Usage: $0 <NAME.conf> <FAILURE CMD>
#
# NAME.conf is a shell snippet that defines:
# NS_FQDN - your FQDN e.g. "example.com"
# NS_NAME - hostname to perform the update on, e.g. "mysub" to update
#   "mysub.example.com" or "@" to update "example.com".
# IPV - version of IP to use, either 4 or 6
#
# NAME.key must exist and contain a valid gandi API key
...
. "$(readlink -f "$CONF")"
test -n "$NS_FQDN" || { echo >&2 "conf didn't define NS_FQDN"; exit 1; }
...
! curl -s -H "Authorization: Apikey $(cat "$BASE.key")" \
  "https://api.gandi.net/v5/livedns/domains/$NS_FQDN/records/$NS_NAME" \
  -X PUT \
  -H "Content-Type: application/json" \
  -d '{"items": [{"rrset_type": "'"$IPTYPE"'", "rrset_ttl": 1200, "rrset_values": ["'"$IPADDR"'"]}]}' \
